...
let chargeRefund = function(transaction, order) {
	if (transaction === 0) {
		...
		// push refunded charge object to refunds:[]
		order.refunds.push({
		...
		// [optional]
		const refundCollection = order.refunds;
		const chargeCollection = order.charges;
		// compare the cost of refund and charge
		refundCollection.forEach((refObj) => {
			chargeCollection.forEach((chaObj) => {
				if (refObj['charge_id'] === chaObj['id']) {
					// see if the charge has fully refunded
					let amountDif = refObj['amount'] + chaObj['amount'];
					// when it is not fully refunded, 
					// see if refund costs more than the money charges from customer
					if (amountDif !== 0) {
						const refundCost = Math.abs(refObj['amount']) + 0.3;
						const charged = chaObj['amount'];

						if (refundCost > charged) {
							refObj['amount'] = chaObj['amount'] * -1;
						}
					}
				}
			});
...
// Test Case 6 [optional]
// New transaction: -0.8
let order6 = {
	charges: [ { id: '5cdd802nasd10002', amount: 1 }, { id: '5cdd802nasd10003', amount: 1 } ],
	refunds: [ { charge_id: '5cdd802nasd10002', amount: -1 } ]
};
...
console.log(chargeRefund(-0.8, order6));
